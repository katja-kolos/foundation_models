...
def save_to_csv(setting, model_name, entry, split):
    model_name = model_name.lower().replace("/", "_").replace(".", "_")
    
    results_save_path = f"./benchmarking/{model_name}"
    os.makedirs(results_save_path, exist_ok=True)

    file_path = f"{results_save_path}/{model_name}_{split}_output_setting_{setting}.csv"
...
def process_datapoint_at_setting(example, idx, setting, llm, model_name, sampling_params, resumed_index_from, split):
    ...
    #prompt_answer_and_solution = '\nAnswer the question and explain your solution'

    text_input = setting_templates[setting].format(
        question=example["question"], 
        task=example["task"], 
        choice=example["choices"], 
        context=example["hint"] if example["hint"] else 'Unavailable', 
        lecture=example["lecture"] if example["lecture"] else 'Unavailable', 
        solution=example["solution"] if example["solution"] else 'Unavailable', 
    ...
    # Test 1: no system prompt -- model tries to explain back what's happenning but does not answer the question
    #text = text_input 
    
    # Test 2:
    #system_prompt = "You see a school exam task and a related image. Use your knowledge and reasoning to answer the question."
    #text = f"{system_prompt}\n{text_input}"

    #Test 3:
    #text = f"reasoning task || {text_input} || answer en"
    
    #Test 4: minimal (remove Instruction)
    #text = f"{text_input}\nAnswer en: "

    #Test 5: also minimal, starts with JSON
    #text = f"THIS IS A PROBLEM WITH TASK, IMAGE, CORRECT ANSWER AND SOLUTION.\n{text_input}\n Solution: "

    #Test 6:
    system_prompt = "You are a highly knowledgeable assistant trained in multimodal reasoning and capable of solving science-related questions with textual and visual data. Your task is to answer science questions accurately based on the provided context and image, if available."
    text = f"{system_prompt}\n{text_input}"
    ...
    save_to_csv(setting, model_name, entry, split)
...
def main():
    ...
    parser.add_argument(
        ...
        help="The name of the model to download and use. Default: google/paligemma2-3b-pt-224",
    )
    parser.add_argument(
        "--ngpus",
        type=int,
    ...
    login_to_hf() #not necessary for qwen, required for pixtral and paligemma
    ...
    data.map(lambda example, idx: process_datapoint_at_setting(
        ...
        split=args.split), with_indices=True)
