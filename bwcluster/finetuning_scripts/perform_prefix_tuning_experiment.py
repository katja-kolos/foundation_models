...
def train(model, tokenizer, processor, optimizer, dataloader_train, dataloader_val, preprocess_func, model_name, batch_size, exp_setting, num_epochs):
    train_errors = []
    val_errors = []
    model.train()
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for prompts, texts, images, y in dataloader_train:
            ...
            if num_samples % 1024 == 0:
                # detailed look into outputs
                logging.info(f"output_text for sample {num_samples}")
        ...
        save_assets(model, model_name, train_errors, val_errors, batch_size, adapter_type='prefix', exp_setting=exp_setting, num_epochs=epoch, saving_mode='adapter_only')
...
def main():
    ...
    train_errors, val_errors = train(model_prefix, tokenizer, processor, optimizer, dataloader_train, dataloader_val, preprocess_input_function, model_name, args.batch_size, args.exp_setting, args.num_epochs)
    
    logging.info('Saving assets')
    save_assets(model_prefix, model_name, train_errors, val_errors, args.batch_size, adapter_type='prefix', exp_setting=args.exp_setting, num_epochs=args.num_epochs, saving_mode='adapter_only')
    logging.info('Done')
