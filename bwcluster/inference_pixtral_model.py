...
def process_datapoint_at_setting(example, idx, setting, llm, model_name, sampling_params, resumed_index_from):
    ...
    if image is not None:
        # i am not sure if i hate vllm developers more than mistral developers or not
        # i hope agi will come and forbid overcomplicating stuff
        tokenizer = llm.llm_engine.tokenizer.tokenizer.mistral 
        ...
        prompt = TokensPrompt(prompt_token_ids=tokenized.tokens, multi_modal_data=mm_data) #already contains special token like [IMG] but not image png
        # inputs = {
        #         "prompt": prompt,
